...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        ...
        // Add or Update Product $navFieldName
        changes = TrackChanges('product');
        if (changes.length > 0 && changes.indexOf('$navFieldName') > -1) {
            data = product.find((x) => x.key === '$navFieldName');
            rslt = await Support.AddOrUpdateDocument(data);
            if (rslt.status) {
                let newImageId = parseInt(rslt.id);
                data = [
                    { key: "Product_id", value: parseInt(productId) },
                    { key: "product${navFieldName}", value: newImageId }
                ];
                rslt = await Support.AddOrUpdateProduct(data, dropDownOptions);
                if (!rslt.status) return;

                let newValues = product.find((x) => x.key === '${navFieldName}').value;
                newValues = { ...newValues, DocId: newImageId };
                row['product'].find((x) => x.key === '${navFieldName}').value = newValues
                row['product'].find((x) => x.key === 'product${navFieldName}').value = newImageId;
                // Update Back for next tracking purpose
